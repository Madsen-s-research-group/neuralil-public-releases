...
class NeuralILASECalculator(Calculator):
    ...
    def __init__(self, model, model_info, max_neighbors, n_devices=None):
        ...
        if (model.model_name != model_info.model_name or
                model.model_version != model_info.model_version):
        ...
        self.symbol_map = {
            s: i
            for i, s in enumerate(model_info.sorted_elements)
        }
    ...
    def _calc_atomic_energies(self, p, t, c):
        ...
        padded_t = padded_t.reshape((self.n_devices, n_atoms // self.n_devices))
    ...
    def calculation_required(self, atoms, properties):
        ...
        return len(
            compare_atoms(
                self.atoms, atoms, excluded_properties=self.excluded_properties
            )
        ) > 0
